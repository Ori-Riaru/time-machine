window.onload = () => {
  ...
  if (settings && event) {
    ...
    (<HTMLInputElement>document.getElementById("sqwimble")!).value =
      settings.sqwimble;
    (<HTMLInputElement>document.getElementById("latitude")!).value =
      event.location.latitude;
    (<HTMLInputElement>document.getElementById("longitude")!).value =
      event.location.longitude;
    (<HTMLInputElement>document.getElementById("deterministic")!).checked =
      settings.deterministic;
    (<HTMLInputElement>document.getElementById("assume")!).checked =
      settings.assume;
  }
};
...
document.getElementById("start")?.addEventListener("click", () => {
  let settings = {
    ...
    deterministic: (<HTMLInputElement>document.getElementById("deterministic"))
      ?.checked,
